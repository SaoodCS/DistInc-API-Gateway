   ...
   static verifyHeaders(
      ...
      if (!contentType) {
         return res
            ...
            .send(`${resCodes.BAD_REQUEST.prefix}: Missing Content-Type Header In Request`);
      }
      if (contentType !== 'application/json') {
         return res
            .status(resCodes.BAD_REQUEST.code)
            .send(`${resCodes.BAD_REQUEST.prefix} Invalid Content-Type Header In Request`);
      }
      ...
      if (!serviceReq) {
         return res
            .status(resCodes.BAD_REQUEST.code)
            .send(`${resCodes.BAD_REQUEST.prefix}: Missing Microservice In Request`);
      }
      if (typeof serviceReq !== 'string') {
         return res
            .status(resCodes.BAD_REQUEST.code)
            .send(`${resCodes.BAD_REQUEST.prefix}: Invalid Microservice In Request`);
      }

      const serviceReqObj = ArrayOfObjects.objectWithVal(microservices, serviceReq);
      if (!serviceReqObj) {
         return res
            .status(resCodes.BAD_REQUEST.code)
            .send(`${resCodes.BAD_REQUEST.prefix}: Requested Microservice Not Found`);
      }
   ...
   static async verifyAuthToken(
      ...
      try {
         ...
         if (!authHeader) {
            return res
               .status(resCodes.BAD_REQUEST.code)
               .send(`${resCodes.BAD_REQUEST.prefix}: Missing Auth Header In Request`);
         }
         ...
         return decodedToken
            ...
            : res
                 ...
                 .send(`${resCodes.UNAUTHORIZED.prefix}: Unauthorized Token`);
      } catch (err) {
